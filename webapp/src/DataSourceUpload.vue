<template>
    <span>
        <span v-if="source.upload && source.upload.sources">
            <span v-if="Object.keys(source.upload.sources).length > 1">
                <div id="srcs" class="ui top attached tabular menu">
                    <a :class="['green item', i === 0 ? 'active' : '']" :data-tab="'upload_' + subsrc"
            ...
            <div :class="['ui bottom attached tab segment', i === 0 ? 'active' : '']" :data-tab="'upload_' + subsrc"
                v-for="(info, subsrc, i) in source.upload.sources" :key="i">
                <div class="ui two grid">
                    <div class="row">
                        <div class="ten wide column">
                            <table class="ui small very compact definition collapsing table">
                                <tbody>
                                    ...
                                    <tr>
                                        <td>Status</td>
                                        <td><i :class="info.status == 'failed' ? 'red' : 'green'">{{ info.status }}</i>
                                        </td>
                                    </tr>
                                    <tr v-if="info.error">
                                        <td>Error</td>
                                    ...
                                    <tr>
                                        <td>Last Upload</td>
                                        <td>{{ info.started_at }} <i v-if="info.started_at">({{ info.started_at |
                                            moment("from", "now") }})</i></td>
                                    </tr>
                                    <tr>
                                        <td>Duration</td>
                                        <td>{{ info.time }}</td>
                        ...
                        <div class="six wide column" v-if="info.uploader">
                            <p v-if="info.uploader.dummy">This is a <i>dummy</i> uploader, meaning data isn't actually
                                uploaded but rather already stored in a collection.
                                In order to register data, a <b>release</b> is required in order the uploader to run
                                properly.</p>
                            <div :class="['ui form', 'upload', actionable, subsrc]">
                                <!-- FIRST ROW (inline fields) -->
                                <div class="inline fields">
                                    <div class="required field" v-if="info.uploader.dummy">
                                        <input type="text" placeholder="Specify a release" v-model="release">
                                    </div>

                                    <div class="field">
                                        <button :class="[
                                            'ui labeled small icon button',
                                ...
                                <!-- SECOND ROW (Update Metadata) -->
                                <div class="field">
                                    <button :class="[
                                        'ui labeled small icon button teal update-source-meta',
                                        info.status == 'uploading' ? 'disabled' : ''
                                    ]" @click="do_update_source_meta(subsrc)" :data-subsrc="subsrc">
        ...
        <div class="ui modal" id="metadata-confirming">
            ...
            <div class="content">
                <div class="ui text small red" style="text-align: center; margin-bottom: 1rem;">Warning! Typically you
                    want to re-run "upload" step to update both the metadata and actual data at the same time. Please
                    proceed to update the metadata only if it won't cause any inconsistency with the previously uploaded
                    data.</div>

                <div v-for="builder_class in metadataCompareData" v-bind:key="builder_class.kclass" class="ui grid">
                    <div class="row" style="padding-bottom: 0;">
                        <h3 v-for="version in ['current', 'new']" v-bind:key="version"
                            :class="'eight wide column ' + version">
                            {{ version }} Metadata
                        </h3>
                    </div>
                    <div class="row metadata-wrapper">
                        <pre v-for="version in ['current', 'new']" v-bind:key="version"
