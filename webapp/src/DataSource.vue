        ...
        <div class="content">
            ...
            <div class="left aligned header word-wrap" v-if="source.name">
                <router-link :to="'/source/' + source._id" class="ui blue header">
                    <h3>
                        {{ source.name }} <i v-if="source.data_plugin && source.data_plugin.plugin.loader == 'advanced' " title="Advanced Plugin" class="plugin small star icon advanced"></i>
                    </h3>
                </router-link>
                <!-- error -->
            ...
            <div class="meta" style="overflow:hidden">
                <div v-if="source.download && source.download.started_at">
                    <small class="time"><i class="clock icon outline"></i> Updated {{ source.download.started_at | moment("from", "now") }}</small>
                </div>
                <div v-else>
        ...
        <div class="extra content light-grey" :class="actionable">
            ...
            <span v-else>
                <div class="ui icon buttons left floated mini">
                    <button class="ui button m-1" data-tooltip="Download Data" v-on:click="do_dump" v-bind:class="{ yellow: (download_status == 'downloading')}" v-if="source.download">
                        <i class="download cloud icon labeled" data-content="Dump" ></i>
                    </button>
                </div>
                <div class="ui icon buttons left floated mini">
                    <button class="ui button m-1" data-tooltip="Upload Data" v-on:click="do_upload" v-bind:class="{ yellow: (upload_status == 'uploading')}" v-if="source.upload">
                        <i class="database icon labeled" data-content="Upload"></i>
                    </button>
                </div>
                <div class="ui icon buttons left floated mini">
                    <button class="ui button m-1" data-tooltip="Inspect Data" v-on:click="inspect" v-bind:class="{ yellow: (inspect_status == 'inspecting')}">
                        <i class="unhide icon labeled" data-content="Inspect"></i>
                    </button>
                </div>
            </span>
            <div class="ui icon buttons right floated mini">
                <button class="ui button delete-btn" data-tooltip="Delete Source" v-on:click="unregister" v-if="source.data_plugin">
