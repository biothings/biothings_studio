                    ...
                    <div class="six wide column">
                        ...
                        <p>
                          <div class="six wide field">
                            <textarea class="json" rows="5" id="submap" v-model="strsubmap"></textarea>
...
export default {
  ...
  computed: {
    ...
    strsubmap: {
      get: function () {
        if (this.tmp_strsubmap) { return this.tmp_strsubmap } else { return JSON.stringify(this.submap, null, 2) }
      },
      set: function (val) {
        try {
    ...
    indexed: {
      ...
      set: function (val) {
        if (val) { $(`#modal_${this.name}-${this.map_id} .copy_to_all`).removeClass('disabled') } else { $(`#modal_${this.name}-${this.map_id} .copy_to_all`).addClass('disabled') }
        this.updateSubMap()
      }
    },
    copied_to_all: {
  ...
  methods: {
    generateField: function (map_id, name, path, leaf) {
      if (leaf) { return `<a class='mapkey leaf' id='${path}/${name}' map_id='${map_id}'>${name}</a>` } else { return `<span class='non-leaf'>${name}</span>` }
    },
    ...
    updateSubMap: function () {
      ...
      // copy to all ?
      if (copy_to_all) { Vue.set(this.submap, 'copy_to', ['all']) } else { Vue.delete(this.submap, 'copy_to') }
      // index or not
      if (index) { Vue.delete(this.submap, 'index') } else {
    ...
    modifyMapKey: function (event) {
      ...
      if (`${this.name}-${this.map_id}` != map_id) { return }
    ...
    htmlmap: function () {
      ...
      if (!this.read_only) {
        ...
        traverse(html, function (key, val, obj, path) {
          if (typeof val === 'object' && ['properties', 'copy_to'].indexOf(key) == -1) {
            var leaf = true
            if ('properties' in obj[key]) { leaf = false }
      ...
      { $(`#${this.name}-${this.map_id}`).text(JSON.stringify(this.map['pre-mapping'], null, 4)) } else {
